...
export default class FigletDoomFont {
    figChars;
    isLoaded;
    horizontalSmushLengthMemo;
    horizontalSmush1(char1, char2) {
        if (char1 === char2 && char1 !== OPTIONS.HARD_BLANK) {
            return char1;
        }
        return false;
    }
    horizontalSmush2(char1, char2) {
    ...
    getHorizontalSmushLength(text1, text2) {
    ...
    horizontalSmush(textLines1, textLines2, overlap) {
        const output = [];
    ...
    generateLineOfFigText(text) {
        let outputText = new Array(OPTIONS.height).fill("");
